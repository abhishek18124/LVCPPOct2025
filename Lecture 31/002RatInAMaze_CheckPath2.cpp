...
bool f(char maze[][10], int m, int n, int i, int j) {
	...
	// f(0, 0) : it is fn that check if there is a path from (0, 0)th cell to (m-1, n-1)th cell

	// f(i, j) : it is fn that check if there is a path from (i, j)th cell to (m-1, n-1)th cell
