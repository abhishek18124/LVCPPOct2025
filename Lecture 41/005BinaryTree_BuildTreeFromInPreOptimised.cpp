...
TreeNode* buildTree(const vector<int>& preOrder,
	...
	int keyIndex = indexMap[key];

	// ask your friend to build the leftSubtree
	// from its inOrder[s...keyIndex-1]

	root->left = buildTree(preOrder, inOrder, s, keyIndex - 1, indexMap);

	// ask your friend to build the rightSubtree
	// from its inOrder[keyIndex+1...e]

	root->right = buildTree(preOrder, inOrder, keyIndex + 1, e, indexMap);
...
int main() {
	...
	TreeNode* root = buildTree(preOrder, inOrder, 0, n - 1, indexMap);
