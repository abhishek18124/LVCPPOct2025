...
// time : O(n+m) where n is the length of the 1st linkedList and m is the length of the 2nd linkedList
// space: O(1)
// [HW] try to solve this using recursion
...
ListNode* mergeSortedLinkedList(ListNode* head1, ListNode* head2) {
	...
	if (head1 != NULL) {
		// 1st linkedList is left, 2nd linkedList is exhaused
		temp->next = head1;
	}

	if (head2 != NULL) {
		// 1st linkedList is exhaused, 2nd linkedList is left
		temp->next = head2;
	}
