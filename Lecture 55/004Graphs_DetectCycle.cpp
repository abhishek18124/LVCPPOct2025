	...
	for (int ngb : adj[cur]) {
		...
		if (!vis[ngb]) {
			...
			if (dfs(ngb, cur, adj, vis)) {
				...
				// which means we've found a cycle in the component
				// of cur
			...
			// check if the edge between cur and ngb
			...
			if (par != ngb) {
				...
				// therefore we've found a cycle in the
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (!vis[i]) {
			...
			// check if a cycle is present in the component
			// of node i using dfs(...)
